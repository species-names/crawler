      ...
      while ((linksRaw = constLinkRegex.exec(preparedText)) !== null) {
        ...
        linksRaw.forEach(function (link) {
          ...
          if (splitted.length > 1) {
            const firstPart = splitted[0].replace(/\{/gm, '').replace(/\}/gm, '').replace(/\[/gm, '')
            .replace(/\]/gm, '').trim();
            let linkName = splitted[1].replace(/\}/gm, '').replace(/\[/gm, '')
            .replace(/\]/gm, '').trim();
            if ((firstPart === 'splast' || firstPart === 'sp') && splitted.length > 3) {
              linkName += splitted[2];
            }
            links.push(linkName);
          }
