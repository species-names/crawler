...
module.exports.parseLinksFromQueryData = function (regexTerm, data) {
  ...
  if (text) {
    // text = text.replace(/(?:\r\n|\r|\n)/g, '');
    // https://species.wikimedia.org/w/api.php?action=query&prop=revisions&rvlimit=1&rvprop=content&titles=Casuariiformes&format=json
    ...
    // https://species.wikimedia.org/w/api.php?action=query&prop=revisions&rvlimit=1&rvprop=content&titles=Trogonidae&format=json
